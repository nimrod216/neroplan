...
class DoubleSequential(nn.Sequential):
    def forward(self, x, adj_adjust):
        for idx,module in enumerate(self._modules.values()):
            # print(f'layer no.{idx}')
            x = module.forward(h_0=x, adj_adjust=adj_adjust)
        return x
...
# def layer_init(layer, std=np.sqrt(2).item(), bias_const=0.0):
    # torch.nn.init.(layer.weight, std)
...
class GCN(nn.Module):
    def __init__(self, feature_num, ip_node_num, n_hidden, num_layer):
        ...
        # print("num of gcn layer:{}".format(len(self.gcn_list)))
        self.gcn_list = DoubleSequential(*self.gcn_list)
    ...
    def forward(self, obs):
        ...
        h_0 = self.gcn_list.forward(x=h_0, adj_adjust=adj_adjust)
